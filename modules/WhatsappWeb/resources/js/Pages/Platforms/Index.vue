...
const verifyNumber = (uuid) => {
  axios.get(route('user.whatsapp-web.api.platforms.check-verification', uuid))
    .then((res) => {
      toastComposable.success(trans('Number is verified successfully'))
      router.reload()
    })
    .catch((err) => {
      toastComposable.danger(err.response?.data?.message ?? trans('Something went wrong'))
    })
}
</script>
...
<template>
  ...
  <div class="mt-4 flex w-full flex-col gap-4">
    <div v-if="platforms && platforms?.data.length" class="grid grid-cols-1 gap-4 xl:grid-cols-2 2xl:grid-cols-3">
      <div v-for="(platform, index) in platforms.data" :key="index"
        ...
        <div class="flex flex-col items-start justify-between gap-2 sm:flex-row sm:items-center">
          <div class="flex items-center gap-4">
            <div class="flex flex-col">
              ...
              <span class="mt-1 inline-flex w-fit items-center rounded-full px-2 py-px text-xs lowercase" :class="{
                'bg-emerald-100 text-emerald-700':
                  platform.status === 'authenticated' || platform.status === 'verified' || platform.status === 'authenticated',
                ...
                <span class="mr-1 mt-0.5 h-2 w-2 rounded-full" :class="{
                  'bg-emerald-500':
                    platform.status === 'authenticated' || platform.status === 'verified',
          ...
          <div class="mt-4 flex items-center justify-around gap-2">
            <Link
              ...
              :href="route('user.whatsapp-web.platforms.conversations.index', platform.uuid)" title="View Messages">
            <Icon icon="bx:message-square-dots" class="text-xl" />
            </Link>

            <Link title="Reconnect"
            ...
            <button v-if="!platform.meta?.verified" title="Verify Number" type="button"
            ...
            <button
              class="inline-flex items-center justify-center rounded-lg bg-gray-50 p-2.5 text-gray-700 transition-colors hover:bg-gray-100 hover:text-emerald-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-emerald-400"
              @click="() => openPlatformSettingModal(platform)" title="Settings">
              <Icon icon="bx:cog" class="text-xl" />
            </button>
            <button title="Delete Platform" type="button"
        ...
        <div class="mt-4 grid grid-cols-2 gap-2 md:grid-cols-3">
          ...
          <div class="flex flex-col rounded-lg bg-gray-50 p-3 dark:bg-gray-800/50">
            <span class="text-xs font-medium text-gray-500">{{ trans('External Webhook') }}</span>
            <span class="text-sm font-semibold"
        ...
        <div v-if="platform.meta?.webhook_callback_url" class="mt-2 rounded-lg bg-blue-50 p-3 dark:bg-blue-900/20">
        ...
        <div v-if="platform.meta?.send_welcome_message" class="mt-2 rounded-lg bg-purple-50 p-3 dark:bg-purple-900/20">
          <div class="flex items-start gap-2">
